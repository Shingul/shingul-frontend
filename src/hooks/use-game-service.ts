...
import { Game, GameAndQuestionsResponse, JoinedMessage, Player } from "../types/api";
    ...
    (msg: GameSessionBroadcast) => {
      ...
      queryClient.setQueryData<Game>(qk.games.detail(gameId), (old) => {
        ...
        return {
          ...
          currentQuestionIndex: msg.currentQuestionIndex ?? old.currentQuestionIndex,
          questionStartedAt: msg.questionStartedAt ?? old.questionStartedAt,
          secondsPerQuestion: msg.timePerQuestionSeconds ?? old.secondsPerQuestion,
...
function useServerTimer(questionStartedAt?: string, secondsPerQuestion?: number) {
...
function useParticipantJoined(gameId: string, page: "participant" | "host") {
  ...
  const queryClient = useQueryClient();
  const onMessage = useCallback((message: JoinedMessage) => {
    ...
    if((message.status === "left" || message.status === "kicked")){
     ...
     if(page === 'participant') toast.success(`${message.nickname} left the game!`);
     } else{
    if(page === "host"){
      queryClient.setQueryData<Game>(qk.games.detail(gameId), (old) => {
        if (!old) return old;
...
function usePublicGameSession(gameId: string) {
  const queryClient = useQueryClient();
  const onMessage = useCallback((message: GameSessionBroadcast) => {
    console.log("message public game session", message);

    queryClient.setQueryData<GameAndQuestionsResponse>(qk.games.gameAndQuestions(gameId), (old: GameAndQuestionsResponse | undefined) => {
      if (!old) return old;
      return {
        ...old,
        game: {
          ...
          currentQuestionIndex: message.currentQuestionIndex ?? old?.game?.currentQuestionIndex,
          questionStartedAt: message.questionStartedAt ?? old?.game?.questionStartedAt,
          secondsPerQuestion: message.timePerQuestionSeconds ?? old?.game?.secondsPerQuestion,
